...
# Varijanta: GraphSAINT – GNN na (celom) grafu; ovde 1 GCN sloj bez uzorkovanja

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data. 
GraphSAINT = GNN na podgrafovima; ovde jedan podgraf = ceo graf, 1 GCN sloj. 
"""
...
def compute_graphsaint_embeddings(M, k=EMBED_DIM):
...
def main():
    ...
    emb = compute_graphsaint_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (GraphSAINT + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (GraphSAINT + kvantna regresija, deterministički, strukturno):
    (7, 8, 11, 23, 26, 33, 34)
...
Jedan „podgraf” = ceo graf (bez nasumičnog uzorkovanja, 
deterministički, npr. Breadth-First Search).

A_tilde = A + I, A_norm = D^{-1/2} A_tilde D^{-1/2}, 
H0 = prvih k sopstvenih vektora Laplaciana 
(bez 0-og sopstvenog vektora, prvih k-1).

Jedan GCN sloj: H1 = A_norm @ H0 (kao u k_7), 
